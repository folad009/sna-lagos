          ...
          {events.map((event, i) => (
            <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all group">
              ...
              <div className="space-y-2 mb-8 text-sm text-gray-500">
                <p className="flex items-center"><Clock size={16} className="mr-2 text-[#bb6e31]" /> {event.date}</p>
                <p className="flex items-center"><MapPin size={16} className="mr-2 text-[#bb6e31]" /> {event.location}</p>
              </div>
